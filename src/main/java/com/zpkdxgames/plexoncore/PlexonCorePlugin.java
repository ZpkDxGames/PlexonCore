...
public final class PlexonCorePlugin extends JavaPlugin implements Listener {
    ...
    public void onEnable() {
        ...
        int defaultCompute = Math.max(2, Math.min(4, processors / 2));
        int workers = configService.core().integer("executor.worker-threads", defaultCompute);
        int queueCapacity = configService.core().integer("executor.queue-capacity", 4096);
        int ioWorkers = configService.core().integer("executor.io-threads", 1);
        int ioQueueCapacity = configService.core().integer("executor.io-queue-capacity", 2048);
        scheduler = new CoreScheduler(this, workers, queueCapacity, ioWorkers, ioQueueCapacity);
        moduleRegistry = new ModuleRegistry(coreVersion, List.of(CoreVersion.of(1, 0, getPluginMeta().getVersion())));
    ...
    private final class DefaultCoreAPI implements PlexonCoreAPI {
        ...
        @Override public CoreEventGateway events() { return eventGateway; }
        @Override public BlockOriginService blockOrigins() { return blockOriginService; }
    }
}
